...
class PatientSignUpView(CreateView):
    ...
    success_url = reverse_lazy('userlogin:patient_dashboard')
    ...
    def form_valid(self, form):
        user = form.save()
        return redirect('userlogin:patient_dashboard')

class DoctorSignUpView(CreateView):
    model = User
    form_class = DoctorSignUpForm
    template_name = 'userlogin/signup.html'
    success_url = reverse_lazy('userlogin:doctor_dashboard')
    ...
    def form_valid(self, form):
        user = form.save()
        return redirect('userlogin:doctor_dashboard')
...
class PatientLoginView(LoginView):
    ...
    def get_success_url(self):
        return reverse_lazy('userlogin:patient_dashboard')
...
class DoctorLoginView(LoginView):
    ...
    def get_success_url(self):
        return reverse_lazy('userlogin:doctor_dashboard')

class PatientDashboardView(LoginRequiredMixin, CreateView):
    model = User
    template_name = 'userlogin/patient_dashboard.html'
...
class DoctorDashboardView(LoginRequiredMixin, CreateView):
    model = User
    template_name = 'userlogin/doctor_dashboard.html'
    fields = ['first_name', 'last_name', 'email', 'address_line1', 'city', 'state', 'pincode', 'profile_picture']
